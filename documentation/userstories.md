# User Stories

|Done|Itr|Pair|ID|Title|As a...|I want to...|So that...|PP|priority|Progress|Remaining PP|
|-|-|-|-|-|-|-|-|-|-|-|-|
|TRUE|3|NS AA/ SN AA/ GI NJ||import calendar|user|be able to import my calendar from an external source|I can conveniently communicate my availabilities|2.0|M|1.0|0|
|TRUE|3|NS SS/SN SS||edit invites|user|easily add or remove invites while creating or updating event|I can edit who attends if things change|0.5|S|0.5|0|
|TRUE|3|SN SS/GI SS||manage created events|user|change the details of the event, like edit dates or participants|I can update the event as things change and progress|1.5|S|1.5|0|
|TRUE||EO AA||Login|user|Enter the correct username and password of an existing account on the login page|The program checks if the username exists and the password is correct and logs the user into their account|1.0|M|1.0|0|
|TRUE||EO AA||Logout|user|Click the logout button while logged in|Logs the current user out of their account and returns to the login screen|1.0|M|1.0|0|
|TRUE|3|EO GI||delete event|user|delete my event|if the event is no longer happening I can remove it from everyone's schedule|1.0|S|1.0|1|
|FALSE||NS CR||scrollbar|user|scroll up and down/left and right on larger pages|I can see the whole page||1.0|S|1|
|TRUE|3|EO SS|---|Back button|user|leave event manage screen without saving|it's easy to leave if I no longer need to create or update event|0.5|C|0.5|0|
|TRUE|3|EO SS|---|Date picker|user|pick a date using a calendar|I don't need to type the date in manually|0.5|C|0.5|0|
||3|||admin delete|admin|block removal of the admin form the delete user options|admin cant acedentaly remove it self|0.5|M|0.5|0|
|TRUE|3|GI SN|-|Register|user|register to the system|I can be saved to the system and login later on|1|M|1|0|
|TRUE|3|GI SN AA GI|-|Delete My Account|user|delete myself from the system|my previous data from the system will no longer stored|1|M|1|0|
|TRUE|3|EI GI AA GI|-|Remove User|admin|delete selected user other than admin|selected users stored data will be deleten|1|M|1|0|
|True|3| NJ AA NS|-| Save Events|User|See my events after closing the application|2|S|2|0|
|True|1|NS CR|-|Display Invites|User|see incoming invites on home page|I can accept or decline the invitation|1|M|1|0|
|TRUE|1|NJ SS|-|Add Time|User|add a time when creating event|I can set a time for my event|0.5|C|0.5|1|
|TRUE|1|NS GI CR|-|Reject Invite|User|reject an event invite|I don't have to go to events I don't want to go to|0.5|C|0.5|1|
|TRUE|1|NS GI CR|-|Accept Invite|User|accept an event invite|I can accept the invitation and be added to the event|0.5|C|0.5|1|
|TRUE|1|NJ AA|-|Select Invitees|user|select users from the system to invite to my event|I can select users from the system to invite to my event|0.5|C|0.5|1|
|TRUE|2|NS CR|-|Select Time Slot|user|Select the time range my event can be|I can make sure my event is set at an appropriate time of the day, e.g in the working day|0.5|C|0.5|1|
|True|2|NS CR|-|See list of invitees|user|see the list of invitees|I can see who else is attending the event|0.5|C|0.5|1|
|TRUE|3|SN SS||Auto-schedule|user|automatically find a time slot based on all participants' availability|I don't have to manually coordinate meeting times|1.5|M|1.5|0|
|TRUE|3|NS SS||Uninvite|user|remove a participant from my event|I can manage the guest list if someone is no longer required|0.5|S|0.5|0|
|TRUE|3|EO SS||Calendar view|user|see my events in a visual calendar layout|I can get a quick overview of my monthly schedule|1.0|S|1.0|0|
|TRUE|3|EO SS||View event details|user|see full details of an event in a popup|I can understand the event context before attending|0.5|S|0.5|0|

## Legend / notes

|roles:|PP:|priority|
|-|-|-|
|user|1 afternoon of work|M (Must)|
|admin||S (Should)|
|organizer?||C (Could)|
|||W (Would liketo have)|



